...
export function AppSidebar() {
  ...
  return <aside className={cn("fixed left-0 top-0 z-40 h-screen bg-sidebar transition-all duration-300 flex flex-col", isCollapsed ? "w-16" : "w-64")}>
      {/* Header */}
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3">
        <ul className="space-y-1">
          {navigation.map(item => <li key={item.title}>
              {item.href ? <NavLink to={item.href} className={cn("flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all", isActive(item.href) ? "bg-sidebar-primary text-sidebar-primary-foreground" : "text-sidebar-muted hover:bg-sidebar-accent hover:text-sidebar-foreground")}>
                  ...
                  {!isCollapsed && expandedItems.includes(item.title) && item.children && <ul className="mt-1 space-y-1 pl-10">
      ...
      <div className="border-t border-sidebar-border p-3">
        <ul className="space-y-1">
          {bottomNav.map(item => <li key={item.title}>
