...
class BankConnectionSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($connections as $connection) {
            ...
            if (DB::table('bank_connections')->where('account_number', $connection['account_number'])->exists()) {
        ...
        // Seed a sync-failure alert for the EUR StanChart account
        $stanChartId = DB::table('bank_connections')
            ->where('account_number', '0100023400003')
            ->value('id');

        if ($stanChartId) {
            $alertExists = DB::table('bank_alerts')
                ->where('bank_connection_id', $stanChartId)
    ...
    private function seedTransactions(string $connectionId): void
    {
        $transactions = [
            [
                ...
                'amount'            => 85000.00,
                'currency'          => 'GHS',
                'description'       => 'Payment received — CUST-001 INV-2026-000001',
        ...
        foreach ($transactions as $txn) {
            if (DB::table('bank_transactions')->where('transaction_ref', $txn['transaction_ref'])->exists()) {
                continue;
            }
